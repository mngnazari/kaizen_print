...
def is_file_accessible_for_editor(file_order, delay_minutes: int = 0) -> bool:
    ...
    # زمان فعلی با timezone ایران
    now_iran = datetime.now(IRAN_TZ)

    # تبدیل access_time به timezone ایران
    if access_time.tzinfo is None:
        # اگه timezone نداره، فرض کن ایران هست
        access_time_iran = access_time.replace(tzinfo=IRAN_TZ)
    else:
        access_time_iran = access_time.astimezone(IRAN_TZ)

    is_accessible = now_iran >= access_time_iran
    time_diff_minutes = (access_time_iran - now_iran).total_seconds() / 60

    logger.info(f"🕐 زمان فعلی (ایران): {now_iran}")
    logger.info(f"🕐 زمان دسترسی (ایران): {access_time_iran}")
