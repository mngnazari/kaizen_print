        ...
        with database.connection.SessionLocal() as db:
            ...
            if db_order.edit_deadline:
                try:
                    ...
                    if db_order.edit_deadline.tzinfo is None:
                        deadline_iran = db_order.edit_deadline.replace(tzinfo=IRAN_TZ)
                    else:
                        deadline_iran = db_order.edit_deadline.astimezone(IRAN_TZ)

                    logger.info(f"🔍 چک deadline برای سفارش {order_id}:")
                    logger.info(f"  - فایل: {db_order.file_name}")
                    logger.info(f"  - زمان فعلی (ایران): {now_iran}")
                    logger.info(f"  - Edit Deadline (ایران): {deadline_iran}")

                    time_diff_minutes = (deadline_iran - now_iran).total_seconds() / 60
